...
import { MessageCircle, Send, Clock, User, FileText, Calendar } from "lucide-react"
...
export default function CounselorChatSessionPage({ params }: { params: { id: string } }) {
  ...
  const saveSessionNotes = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/appointments/${params.id}/notes`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`
  ...
  const endSession = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/appointments/${params.id}/end`, {
        method: "PATCH",
        headers: {
          "Authorization": `Bearer ${token}`
        }
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  {messages.length === 0 ? (
                    ...
                    messages.map((message) => (
                      <div
                        key={message._id}
                        className={`flex ${message.sender._id === appointment.counselor ? "justify-end" : "justify-start"}`}
                      >
                        <div className="flex items-end space-x-2 max-w-xs lg:max-w-md">
                          {message.sender._id !== appointment.counselor && (
          ...
          <div className="space-y-4">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                />
                <Button onClick={saveSessionNotes} size="sm" className="w-full">
                  Save Notes
                </Button>
              </CardContent>
            </Card>
